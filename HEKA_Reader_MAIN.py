...
import datetime
import pdb
#import dateutil.tz as timeZone
...
class TraceRecord(TreeNode):
    ...
    field_info = [
        ...
        ('InterleaveSizeS', 'i'),#		= fread(fh, 1, 'int32=>int32');%				= 292; (* INT32 *)
        ('InterleaveSkip', 'i'),#		= fread(fh, 1, 'int32=>int32');%				= 296; (* INT32 *)
        ('ImageIndex', 'i'),	#		= fread(fh, 1, 'int32=>int32');%				= 300; (* INT32 *)
        ('Markers', '10d'),		#	= fread(fh, 10, 'double=>double');%				= 304; (* ARRAY[0..9] OF LONGREAL *)
        ('SECM_X', 'd'),		#		= fread(fh, 1, 'double=>double');%				= 384; (* LONGREAL *)
        ('SECM_Y', 'd'),		#		= fread(fh, 1, 'double=>double');%				= 392; (* LONGREAL *)
        ('SECM_Z', 'd'),		#		= fread(fh, 1, 'double=>double');%				= 400; (* LONGREAL *)
        ('TrHolding', 'd'),		#	= fread(fh, 1, 'double=>double');%				= 408; (* LONGREAL *)
        ('TcEnumerator', 'i'),	#	= fread(fh, 1, 'int32=>int32');%				= 416; (* INT32 *)
        ('XTrace', 'i'),		#		= fread(fh, 1, 'int32=>int32');%				= 420; (* INT32 *)
        ('IntSolValue', 'd'),	#	= fread(fh, 1, 'double=>double');%				= 424; (* LONGREAL *)
        ('ExtSolValue', 'd'),	#	= fread(fh, 1, 'double=>double');%				= 432; (* LONGREAL *)
        ('IntSolName', '32s',cstr),	#		= deblank(fread(fh, 32, 'uint8=>char')');%      = 440; (* String32Size *)
        ('ExtSolName', '32s',cstr),	#		= deblank(fread(fh, 32, 'uint8=>char')');%      = 472; (* String32Size *)
        ('DataPedestal', 'd'),	#	= fread(fh, 1, 'double=>double');%				= 504; (* LONGREAL *)
...
class  ChannelRecord(TreeNode):
    ...
    field_info = [
            ...
            ('LinkedChannel','i'),#	= fread(fh, 1, 'int32=>int32');%				=   4; (* INT32 *)
            ('CompressionFactor','i'),#	= fread(fh, 1, 'int32=>int32');%				=   8; (* INT32 *)
            ('YUnit','8s',cstr),	#			= deblank(fread(fh, 8, 'uint8=>char')');%       =  12; (* String8Type *)
            ('AdcChannel','h'),		#		= fread(fh, 1, 'int16=>int16');%				=  20; (* INT16 *)
            ('AdcMode','c'),		#			= fread(fh, 1, 'uint8=>uint8');%				=  22; (* BYTE *)
            ('DoWrite','b'),		#			= fread(fh, 1, 'uint8=>logical');%           	=  23; (* BOOLEAN *)
            ('stLeakStore','c'),	#			= fread(fh, 1, 'uint8=>uint8');%				=  24; (* BYTE *)
            ('AmplMode','c'),		#		= fread(fh, 1, 'uint8=>uint8');%				=  25; (* BYTE *)
            ('OwnSegTime','c'),			#	= fread(fh, 1, 'uint8=>logical');%				=  26; (* BOOLEAN *)
            ('SetLastSegVmemb','c'),		#	= fread(fh, 1, 'uint8=>logical');%				=  27; (* BOOLEAN *)
            ('DacChannel','h'),		#		= fread(fh, 1, 'int16=>int16');%				=  28; (* INT16 *)
            ('DacMode','c'),		#			= fread(fh, 1, 'uint8=>uint8');%				=  30; (* BYTE *)
            ('HasLockInSquare','c'),	#		= fread(fh, 1, 'uint8=>uint8');%				=  31; (* BYTE *)
            ('RelevantXSegment','i'),	#	= fread(fh, 1, 'int32=>int32');%				=  32; (* INT32 *)
            ('RelevantYSegment','i'),		#= fread(fh, 1, 'int32=>int32');%				=  36; (* INT32 *)
            ('DacUnit','8s',cstr),			#		= deblank(fread(fh, 8, 'uint8=>char')');%       =  40; (* String8Type *)
            ('Holding','d'),			#		= fread(fh, 1, 'double=>double') ;%				=  48; (* LONGREAL *)
            ('LeakHolding','d'),		#		= fread(fh, 1, 'double=>double') ;%				=  56; (* LONGREAL *)
            ('LeakSize','d'),			#	= fread(fh, 1, 'double=>double') ;%				=  64; (* LONGREAL *)
            ('LeakHoldMode','c'),		#	= fread(fh, 1, 'uint8=>uint8');%				=  72; (* BYTE *)
            ('LeakAlternate','c'),		#	= fread(fh, 1, 'uint8=>logical');%				=  73; (* BOOLEAN *)
            ('AltLeakAveraging','c'),		#= fread(fh, 1, 'uint8=>logical');%				=  74; (* BOOLEAN *)
            ('LeakPulseOn','c'),		#			= fread(fh, 1, 'uint8=>logical');%				=  75; (* BOOLEAN *)
            ('StimToDacID','h'),		#			= fread(fh, 1, 'int16=>int16');%				=  76; (* SET16 *)
            ('CompressionMode','h'),		#		= fread(fh, 1, 'int16=>int16');%				=  78; (* SET16 *)
            ('CompressionSkip','i'),		#		= fread(fh, 1, 'int32=>int32');%				=  80; (* INT32 *)
            ('DacBit','h'),		#				= fread(fh, 1, 'int16=>int16');%            	=  84; (* INT16 *)
            ('HasLockInSine','c'),		#		= fread(fh, 1, 'uint8=>logical');%				=  86; (* BOOLEAN *)
            ...
            ('Sine_Cycle','d'),		#			= fread(fh, 1, 'double=>double') ;%				=  96; (* LONGREAL *)
            ('Sine_Amplitude','d'),		#		= fread(fh, 1, 'double=>double') ;%				= 104; (* LONGREAL *)            
            ('LockIn_VReversal','d'),		#	= fread(fh, 1, 'double=>double') ;%				= 112; (* LONGREAL *)
            ('Chirp_StartFreq','d'),		#		= fread(fh, 1, 'double=>double') ;%				= 120; (* LONGREAL *)
            ('Chirp_EndFreq','d'),		#		= fread(fh, 1, 'double=>double') ;%				= 128; (* LONGREAL *)
            ('Chirp_MinPoints','d'),		#		= fread(fh, 1, 'double=>double') ;%				= 136; (* LONGREAL *)
            ('Square_NegAmpl','d'),		#		= fread(fh, 1, 'double=>double') ;%				= 144; (* LONGREAL *)
            ('Square_DurFactor','d'),		#	= fread(fh, 1, 'double=>double') ;%				= 152; (* LONGREAL *)
            ('LockIn_Skip','i'),		#			= fread(fh, 1, 'int32=>int32');%				= 160; (* INT32 *)
            ('Photo_MaxCycles','i'),		#		= fread(fh, 1, 'int32=>int32');%				= 164; (* INT32 *)
            ('Photo_SegmentNo','i'),		#		= fread(fh, 1, 'int32=>int32');%				= 168; (* INT32 *)
            ('LockIn_AvgCycles','i'),		#	= fread(fh, 1, 'int32=>int32');%				= 172; (* INT32 *)
            ('Imaging_RoiNo','i'),		#		= fread(fh, 1, 'int32=>int32');%				= 176; (* INT32 *)
            ('Chirp_Skip','i'),		#			= fread(fh, 1, 'int32=>int32');%				= 180; (* INT32 *)
            ('Chirp_Amplitude','d'),		#		= fread(fh, 1, 'double=>double');%				= 184; (* LONGREAL *)            
            ('Photo_Adapt','c'),		#			= fread(fh, 1, 'uint8=>uint8');%				= 192; (* BYTE *)
            ('Sine_Kind','c'),		#			= fread(fh, 1, 'uint8=>uint8');%				= 193; (* BYTE *)
            ('Chirp_PreChirp','c'),		#		= fread(fh, 1, 'uint8=>uint8');%				= 194; (* BYTE *)
            ('Sine_Source','c'),		#			= fread(fh, 1, 'uint8=>uint8');%				= 195; (* BYTE *)
            ('Square_NegSource','c'),		#	= fread(fh, 1, 'uint8=>uint8');%				= 196; (* BYTE *)
            ('Square_PosSource','c'),		#	= fread(fh, 1, 'uint8=>uint8');%				= 197; (* BYTE *)
            ('Chirp_Kind','c'),		#			= fread(fh, 1, 'uint8=>uint8');%				= 198; (* BYTE *)
            ('Chirp_Source','c'),		#		= fread(fh, 1, 'uint8=>uint8');%				= 199; (* BYTE *)           
            ('DacOffset','d'),		#			= fread(fh, 1, 'double=>double') ;%				= 200; (* LONGREAL *)
            ('AdcOffset','d'),		#			= fread(fh, 1, 'double=>double') ;%				= 208; (* LONGREAL *)
            ('TraceMathFormat','c'),		#		= fread(fh, 1, 'uint8=>uint8');%				= 216; (* BYTE *)
            ('HasChirp','c'),		#			= fread(fh, 1, 'uint8=>logical');%				= 217; (* BOOLEAN *)
            ('Square_Kind','c'),		#			= fread(fh, 1, 'uint8=>uint8');%				= 218; (* BYTE *)
            ('Filler1','5s',cstr),		#			= fread(fh, 5, 'uint8=>char');%					= 219; (* ARRAY[0..5] OF CHAR *)           
            ('Square_BaseIncr','d'),		#		= fread(fh, 1, 'double=>double');%				= 224; (* LONGREAL *)
            ('Square_Cycle','d'),		#		= fread(fh, 1, 'double=>double') ;%				= 232; (* LONGREAL *)            
            ('Square_PosAmpl','d'),		#		= fread(fh, 1, 'double=>double') ;%				= 240; (* LONGREAL *)
            ('CompressionOffset','i'),		#	= fread(fh, 1, 'int32=>int32');%				= 248; (* INT32 *)
            ('PhotoMode','i'),		#			= fread(fh, 1, 'int32=>int32');%				= 252; (* INT32 *)
            ('BreakLevel','d'),		#			= fread(fh, 1, 'double=>double') ;%				= 256; (* LONGREAL *)
            ('TraceMath','128s',cstr),		#			= deblank(fread(fh,128,'uint8=>char')');%       = 264; (* String128Type *)
#            ('OldCRC','i'),		#				= fread(fh, 1, 'int32=>int32');%                = 268; (* CARD32 *)
            ('Filler2','i'),		#			= fread(fh, 1, 'int32=>int32');%				= 392; (* INT32 *)
            ('CRC','i'),		#					= fread(fh, 1, 'int32=>int32');%                = 396; (* CARD32 *)
     ]
    size_check = 400
...
#        print(self.header)
        ...
        if self.header.Version[:7] == 'v2x90.2':  ## can not handle files older than v2x90.2
            self.fileVersion = 9
        else:
...
#        print(item.Start, item.Length)
    ...
    def stim(self, index):
        ...
        if groupID >= 1: ## if this is not the first group, need to check accumated series index
            for g in range(groupID -1):
                stimID = stimID + len(self.pul[g].children)  ## get number of series in this group
...
#            print(traceRecord)
        ...
        ## the current value
        for idx, seg in enumerate(chanRecord.children): ## go through segment one by one
...
#            seg_dT = seg.DeltaTIncrement*seg.DeltaTFactor ## probably not necessary for stepwise type of stimuli
            ...
            stim_ = {'EntryName': stimulatingElectrode, 'start': cumSamples, 
             ...
             'Vholding': Holding, 'StimToDac':stim2dacType,'sealresistance':sealResistance}
